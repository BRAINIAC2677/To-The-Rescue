...
void updateSerial()
{
  ...
  {
    gotMsg += mySerial.read();
  }
  if (gotMsg.length() > 0)
  {
    Serial.println(gotMsg); //Forward what Software Serial received to Serial Port
    updCommand(gotMsg);
  }
}
...
{
  if (gotMsg.substring(gotMsg.length() - 5) == "status")
  {
    command = STATUS;
  }
  else if (gotMsg.substring(gotMsg.length() - 2) == "on")
  {
    command = SWITCHON;
  }
  else if (gotMsg.substring(gotMsg.length() - 3) == "off")
  {
    command = SWITCHOFF;
  }
  handleCommand();
...
void loop()
{
  ...
  if (gasAnalogRead <= gasReadLimit and flameRead1 == HIGH and flameRead2 == HIGH)
  {
    Serial.println("n");
